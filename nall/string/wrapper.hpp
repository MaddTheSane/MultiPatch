...
size_t string::capacity() const { return size; }
...
optional<size_t> string::position(const char *key) const { return strpos(data, key); }
optional<size_t> string::iposition(const char *key) const { return istrpos(data, key); }
optional<size_t> string::qposition(const char *key) const { return qstrpos(data, key); }
optional<size_t> string::iqposition(const char *key) const { return iqstrpos(data, key); }
